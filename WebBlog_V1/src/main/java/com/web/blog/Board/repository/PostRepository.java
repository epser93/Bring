...
//    @Query(value = "select distinct * from post where board_id = :board_id and (subject like concat('%',:keyword,'%') or content like concat('%',:keyword,'%'))", nativeQuery = true)
    ...
    List<Post> findAllByMember_NicknameAndBoard_NameNotLikeAndCreatedAtLessThanEqualOrderByPostIdDesc(String writer, String notlike, LocalDateTime date);
...
//    @Query(value = "select distinct * from post where nickname = :nickname and (subject like concat('%',:keyword,'%') or content like concat('%',:keyword,'%'))", nativeQuery = true)
    ...
    //사이트의 모든 포스트 리스트(최신글)
    List<OnlyPostMapping> findByCreatedAtLessThanEqualAndBoard_NameNotLikeOrderByPostIdDesc(LocalDateTime date, String notlike, Pageable pageable);

    //사이트의 모든 포스트 리스트(인기글)
    List<OnlyPostMapping> findDistinctByViewsGreaterThanEqualAndCreatedAtLessThanEqualAndBoard_NameNotLikeOrLikesGreaterThanEqualAndCreatedAtLessThanEqualAndBoard_NameNotLikeOrderByPostIdDesc(int views, LocalDateTime dateV, String notlike1, int likes, LocalDateTime dateL, String notlike2, Pageable pageable);
