...
public interface QpostRepository extends JpaRepository<Qpost, Long> {
    List<OnlyQpostMapping> findByMember_NicknameOrderByQpostIdDesc(String writer, Pageable pageable);
...
//    @Query(value = "select distinct * from qpost where (subject like concat('%',:keyword,'%') or content like concat('%',:keyword,'%') or writer like concat('%',:keyword,'%'))", nativeQuery = true)
    List<OnlyQpostMapping> findDistinctBySubjectContainingOrContentContainingOrMember_NicknameContainingOrderByQpostIdDesc(String keyword1, String keyword2, String keyword3, Pageable pageable);

    //모든 질문글 제목 검색
    List<OnlyQpostMapping> findBySubjectContainingOrderByQpostIdDesc(String keyword, Pageable pageable);

    //모든 질문글 내용 검색
    List<OnlyQpostMapping> findByContentContainingOrderByQpostIdDesc(String keyword, Pageable pageable);

    //모든 질문글 작성자 검색
    List<OnlyQpostMapping> findByMember_NicknameContainingOrderByQpostIdDesc(String keyword, Pageable pageable);
